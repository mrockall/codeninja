define([
  ...
  'collections/players'
], function (namespace, Backbone, PlayersCollection){
  ...
  obj.Game = Backbone.Model.extend({
    ...
    onConnected: function() {
      this.set('socket_id', this.io.socket.socket.sessionid);
    ...
    playerJoinedRoom: function(data) {
      ...
      if(this.get('role') == "Host" && data.gameId == this.get('game_id')){
        ...
        if(this.Players.length == this.players_per_room){
          ...
          this.io.socket.emit('host:start_countdown', this.get('game_id'));
        }
      } else {
    ...
    getRound: function() {
      ...
      if(this.get('current_round') > this.number_of_rounds){
        this.io.socket.emit('game:over', this.get('game_id'));
      } else {
        this.io.socket.emit('game:get_round', this.get('game_id'));
      }
    },
    ...
    playerAnswer: function(answer) {
      ...
      this.io.socket.emit('player:answer',data);
    },
